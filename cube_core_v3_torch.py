"""
SpatialCube V3 — PyTorch GPU Version
======================================
Author: Ivelin Likov
...
V2 base + Force 6 (Langevin dynamics) — confirmed +20% cosine improvement in Test 7.
  ...
  Force 6 — Langevin dynamics (NEW in V3)
    ...
    Tangent-space projection (removing radial component before renorm) is the
    novel element for unit-norm embedding spaces.
    Test 7 result: +20% mean cosine similarity, negligible forgetting cost.
    Temperature T=0.01 by default; anneal toward 0 for final retrieval.
...
class SpatialCubeV3Torch:
    def __init__(self, cube_size=16, embed_dim=64, seed=42, device=None,
        ...
        self.langevin_T = langevin_T   # noise temperature (0 = disabled)

        print(f"SpatialCubeV3Torch: {cube_size}^3 = {self.n_points:,} | "
              f"dim={embed_dim} | device={self.device} | "
              f"langevin_T={langevin_T}")
    ...
    def learn_batch(self, batch_pairs, alpha=0.01, beta=0.005,
        ...
        # Without /d scaling: 64d → 48°, 768d → 75° (catastrophic).
        if self.langevin_T > 0:
            noise_scale = math.sqrt(2 * self.langevin_T / self.embed_dim)
            noise = torch.randn_like(self.embeddings) * noise_scale
            # Project to tangent plane: remove radial component at each point
            radial = (noise * self.embeddings).sum(1, keepdim=True) * self.embeddings
            noise_tangent = noise - radial                              # (N, D)
            # Only apply to active points (those that received gradient signal)
            self.embeddings[active] = self.embeddings[active] + noise_tangent[active]
    ...
    def load(cls, path):
        ...
        cube = cls(cube_size=meta['cube_size'], embed_dim=meta['embed_dim'],
                   langevin_T=meta.get('langevin_T', 0.01))
